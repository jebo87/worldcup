		...
		return (
			<div className={this.state.displayMenu ? 'header header_responsive' : 'header header_normal'}>
				<div className="header_userleft">
					<a href="#" onClick={this.toggleMenu} className="header_icon">
						&#9776;
					</a>
					<div className="name_pic">
						{this.props.user.name || this.props.user.email}
						{this.props.user.image && (
							<img className="header_image" src={this.props.user.image || ball} alt="" />
						)}
					</div>
				</div>

				<div className="links_normal">
					<NavLink className="header_links" to="/" exact={true}>
						Home
					</NavLink>

					{this.props.user.userId && (
						<NavLink className="header_links" to="/leaderboard">
							Leaderboard
						</NavLink>
					)}
				...
				<div id="mySidenav" className={this.state.displayMenu ? 'sidenav sidenav_expanded' : 'sidenav'}>
					<button className="closebtn" onClick={this.toggleMenu}>
						&times;
					</button>
					...
					{this.props.user.userId &&
					this.props.user.admin === true && (
						<NavLink onClick={this.toggleMenu} className="header_links" to="/admin">
							Admin
						</NavLink>
					)}
					{this.props.user.userId && (
